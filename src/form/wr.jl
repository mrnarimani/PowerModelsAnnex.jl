...
function variable_voltage_magnitude_difference_neghboring_buses{T <: QCWRTriVForm}(pm::GenericPowerModel{T})
    ...
    lowerbound = buspairs[bp]["vdiff_min"],
    upperbound = buspairs[bp]["vdiff_max"]
    )
end
...
function variable_voltage_magnitude_diff_sqr{T <: QCWRTriVForm}(pm::GenericPowerModel{T})
    ...
    lowerbound = 0,
    #lowerbound = min((buspairs[bp]["vdiff_min"])^2,(buspairs[bp]["vdiff_max"])^2),
...
## Just like voltage angle difference's bounds, the bpunds for voltage magnitude difference variables should
## also changed for the flipped branches. I didnt want to change buspair in PowerModels base.jl, thus I add the following
## loop to take care of this task.
#    orientations = Set()
#        for (i, branch) in ref(pm, n, :branch)
...
#        end
    ...
    for (i,branch) in ref(pm, n, :branch)
        pair = (branch["f_bus"], branch["t_bus"])
        buspairs = ref(pm, n, :buspairs, pair)


        vdiff_max = buspairs["vdiff_max"]
        vdiff_min = buspairs["vdiff_min"]
...
## following are the constraint that have been added to the model.
#At first I wanter to represent them by function and call the in voltage_constraint function.
#However, I changed my mind and implement them directly in the voltage_constraint function.
